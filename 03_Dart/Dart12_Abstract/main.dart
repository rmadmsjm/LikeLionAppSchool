void main() {
  // Animal 은 추상클래스이므로 객체를 생성할 수 없음
  // Animal animal = Animal(10, "비숑");
  // showInfo(animal);

  // Animal을 구현할 클래스의 객체 생성
  Dog dog = Dog(100, 10, "비숑");
  // Dog은 Animal을 상속받았기 때문에 Animal 타입 변수에 담을 수 있음
  showInfo(dog);
  dog.showRunSpeed();
}

// 개발자가 만든 메서드를 호출하는 함수
// 이럴 경우에는 함수를 받는 매개변수를 정의하면 됨
// 만약 함수의 이름을 정했거나 다수의 함수를 받아서 사용할 경우에는 추상 클래스를 사용하는 것이 더 깔끔함
...
abstract class Animal {
  ...
  // showAge와 showName을 구현하지 않으면 Animal을 사용하는 클래스에서 반드시 재구현 해야 함
  void showAge();
  void showName();
}

// 추상클래스를 사용할 때는 implements 사용
