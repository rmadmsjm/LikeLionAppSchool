void main() {
  ...
  // 포지셔널 매개변수가 정의되어 있다면 매개변수의 순서에 맞춰서 값을 전달
  ...
  // 네임드 매개변수의 경우 순서를 지킬 필요 없음
  var result7 = named_function(a2: 200, a1: 100);
  print("result7 : $result7");

  // required가 붙어 있는 네임드 매개변수는 반드시 값을 지정해야 함
  ...
  // 네임드 파라미터는 순서가 관계가 없으므로 중간의 매개변수를 생략해도 됨
  ...
  // 함수를 매개변수로 받는 함수에 전달할 함수는 익명 함수로 작성하는 경우가 않음
  ...
  // 만약 전달할 함수 내부의 코드가 'return 값' 형태만 있다면 람다식으로 사용이 가능함
  function5(100, 200, (v1, v2){
    return v1 + v2;
  });
...
// 반환값 : 반환값이 있으면 반환값의 타입을 적어주고 없으면 void
int test_function(){
  return 100;
}
...
// requried를 붙이지 않은 매개변수는 필수 항목이 아니기 때문에 개발자가 값을 전달하지 않을 수도 있음
// 이때, null을 허용하는 변수로 정의해야 함
...
// 만약 기본값을 직접 지정한다면 위와 마찬가지로 required 제거
int named_function3({required int a1, int a2 = 2, int a3 = 3}){
  return a1 + a2 + a3;
}

// 포지셔널과 네임드가 섞여 있을 경우 포지셔널을 앞에 작성해야 함
int function4(int a1, {required int a2, int a3=3}){
  return a1 + a2 + a3;
}

// 함수를 매개변수로 받는 함수
// f3 매개변수는 반환타입이 int이고 매개변수가 두 개있는 함수를 받는 매개변수로 정의
