package kr.co.lion.android47_deviceinfo
...
import kr.co.lion.android47_deviceinfo.databinding.ActivityMainBinding
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // 전화 관련된 정보를 관리하는 객체 추출
        val telephonyManager = getSystemService(TELEPHONY_SERVICE) as TelephonyManager

        // 권한 확인이 필요한 일부 코드는 권한 허용 여부를 검사해야 하는 것이 있음
        // checkSelfPermission : 두 번째 매개변수에 넣어준 권한이 허용되어 있는지
        // PERMISSION_GRANTED : 허용된 권한
        // PERMISSION_DENIED : 거부된 권한
        val a1 = ActivityCompat.checkSelfPermission(this, Manifest.permission.READ_PHONE_STATE) == PackageManager.PERMISSION_GRANTED
        val a2 = ActivityCompat.checkSelfPermission(this, Manifest.permission.READ_SMS) == PackageManager.PERMISSION_GRANTED
            ...
            activityMainBinding.apply {
                textView.apply {
                    ...
                    if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
                        ...
                        textView.append("가로 길이 ${w}\n")
                        textView.append("세로 길이 ${h}\n")
                    } else {
                        // 해상도 정보를 담은 객체 추출
                        val point = Point()
                        // 해상도 정보 담기
